...
<!-- 메인 - 공지사항 글 목록 테이블, 이동페이지, 수정, 삭제 -->            
        ...
        <div class="content">
            <table id="list">
                ...
                <tbody>
                    <c:forEach items="${searchList}" var="faqDTO">
                        <tr>
                        	<td><input type="checkbox" name="item" onclick="checkSelectAll()" /></td>
                            <td>${faqDTO.no}</td>
                            <td><a href="/admin/faq/get?no=${faqDTO.no}">${faqDTO.title}</a></td>
                            <td>${faqDTO.reg_date}</td>
                            <td>${faqDTO.update_date}</td>
                            <td><input type="hidden" name="no" value="${faqDTO.no}"></td>
                        </tr>       
                    </c:forEach>    
                </tbody>
            </table>
						    ...
						    <div class="pageInfo_area">
						    	<ul id="pageInfo" class="pageInfo">
									...
									<c:forEach var="num" begin="${__PAGE_MAKER__.startPage}" end="${__PAGE_MAKER__.endPage}">
							        	<li class="searchbutton"><a href="/admin/faq/search?keyword=${__PAGE_MAKER__.cri.keyword}&currPage=${num}&amount=${__PAGE_MAKER__.cri.amount}">${num}</a></li>
							        </c:forEach>
